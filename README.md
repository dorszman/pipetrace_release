...
- **`pipetrace-0.3.3.tar.xz`** — pipetrace source (`v0.3.3` tag), offline-capable (`third_party/` included)
- **`pipetrace-v0.3.3-windows.zip`** — prebuilt Windows viewer (`pipetrace-view.exe` + `pipetrace.app.json` beside the exe)
- **`pipetrace-0.3.2.*` / `pipetrace-v0.3.2-windows.*`**, **`pipetrace-0.3.1.*` / `pipetrace-v0.3.1-windows.*`**, **`pipetrace-0.3.0.*` / `pipetrace-v0.3.0-windows.*`**, **`pipetrace-0.2.0.*` / `pipetrace-v0.2.0-windows.*`**, **`pipetrace-0.1.0.*` / `pipetrace-v0.1.0-windows.*`** — previous releases kept for reference
...
**Windows:** unzip `pipetrace-v0.3.3-windows.zip` and keep `pipetrace.app.json` **beside** `pipetrace-view.exe`. Do **not** double-click the exe alone — open an **analyzed** `.db` (drag-and-drop onto the exe, or Open with). If analyze embedded `app_config_json` into the DB, the sidecar config is optional; otherwise the adjacent `pipetrace.app.json` is required. Fatals and hard crashes write `pipetrace-view.log` next to the exe (startup breadcrumbs + exception codes). Optional checksum: `sha256sum -c pipetrace-v0.3.3-windows.sha256`.
...
sha256sum -c x11-tarballs.sha256
sha256sum -c pipetrace-0.3.3.sha256
sha256sum -c pipetrace-v0.3.3-windows.sha256
```
...
| Path | Size (approx) |
|------|----------------|
| `pipetrace-0.3.3.tar.xz` | 4.4 MiB |
| `pipetrace-v0.3.3-windows.zip` | 1.1 MiB |
| `x11-tarballs/` | 4.6 MiB |
| `scripts/bootstrap_x11_prefix.sh` | — |

License: pipetrace is MIT (see `pipetrace-0.3.3/LICENSE` after extract). X.org tarballs are upstream open source.
